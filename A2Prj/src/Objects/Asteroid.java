...
public class Asteroid extends Movable implements IDrawable, ISelectable, ICollider{
	...
	public Asteroid() {
		...
		for(int i=1; i < sidePoints; i++) {
			...
			xSidePoints[i] = (int)(this.getX() + this.getRandom().nextInt(20) - 10 + size * Math.cos(Math.toRadians(360/sidePoints * i)));
			ySidePoints[i] = (int)(this.getY() + this.getRandom().nextInt(20) - 10 + size * Math.sin(Math.toRadians(360/sidePoints * i)));
